...
export function confirmationEmail(params: {
  ...
  const text = `Hi ${firstName},\n\nYour service appointment is confirmed for ${dateStr}. Rick will be there to take care of ${params.serviceType}.\n\nQuestions? Call 812-564-3719.\n\nThanks for choosing Grease & Threads!`;
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      <h2 style="color: #1e3a5f;">✅ Appointment Confirmed</h2>
      <p>Hi ${firstName},</p>
      <p>Your service appointment is confirmed for <strong>${dateStr}</strong>. Rick will be there to take care of <strong>${params.serviceType}</strong>.</p>
...
export function invoiceEmail(params: {
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <p>Hi ${firstName},</p>
      <p>Your invoice for <strong>${params.serviceType}</strong> is ready.</p>
      <p style="font-size: 1.2em;"><strong>Total: $${params.total.toFixed(2)}</strong></p>
      <p><a href="${params.invoiceUrl}" style="background: #f59e0b; color: white; padding: 10px 20px; text-decoration: none; border-radius: 6px; display: inline-block;">View Invoice</a></p>
...
export function followupEmail(params: {
  ...
  const text = `Hi ${firstName},\n\nIt's been about a month since Rick serviced your ${params.serviceType}. Regular maintenance keeps things running longer and prevents costly breakdowns.\n\nGive us a call at 812-564-3719 or book online: ${bookingLink}\n\n— Grease & Threads`;
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      <h2 style="color: #1e3a5f;">🔧 Time for a tune-up?</h2>
      <p>Hi ${firstName},</p>
      <p>It's been about a month since Rick serviced your <strong>${params.serviceType}</strong>. Regular maintenance keeps things running longer and prevents costly breakdowns.</p>
